  ...
  return (
    <div className="space-y-4 pb-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 bg-white p-4 rounded-3xl border border-slate-200/80 shadow-2xs">
        ...
        {/* View Mode Switcher (Month / Week / Day) & Action */}
        <div className="flex items-center gap-2 shrink-0 flex-wrap">
          <div className="flex items-center bg-slate-100 p-1 rounded-2xl border border-slate-200/80">
            <button
              onClick={() => setViewMode('month')}
              className={`px-3 py-1 rounded-xl text-xs font-black transition-all cursor-pointer ${
                viewMode === 'month' ? 'bg-white text-sky-700 shadow-2xs' : 'text-slate-600 hover:text-slate-900'
              }`}
            >
              ماهانه
            </button>
            <button
              ...
              className={`px-3 py-1 rounded-xl text-xs font-black transition-all cursor-pointer ${
                viewMode === 'week' ? 'bg-white text-sky-700 shadow-2xs' : 'text-slate-600 hover:text-slate-900'
              }`}
            >
              هفتگی
            </button>
            <button
              ...
              className={`px-3 py-1 rounded-xl text-xs font-black transition-all cursor-pointer ${
                viewMode === 'day' ? 'bg-white text-sky-700 shadow-2xs' : 'text-slate-600 hover:text-slate-900'
              }`}
            >
              روزانه (دستورکار)
            </button>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-lg w-full border border-slate-200 shadow-2xl p-6 space-y-5 animate-in fade-in zoom-in-95 duration-150 text-right">
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
              <button
                onClick={() => {
                  onToggleEventStatus(selectedTaskEvent.id);
