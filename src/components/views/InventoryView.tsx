...
import { EquipmentItem, EquipmentComment, AssetClassification, AssetRequirementField, AppUser, EquipmentStatus, ItemKind, CustomEquipmentFilter } from '../../types';
...
export const InventoryView: React.FC<InventoryViewProps> = ({
  ...
  // Manual Registration State - Sequential 3-Level Dropdowns
  const [editingDraftItem, setEditingDraftItem] = useState<EquipmentItem | null>(null);
  const [selectedCatId, setSelectedCatId] = useState<string | null>(null);
  const [selectedSubCatId, setSelectedSubCatId] = useState<string | null>(null);
  const [selectedTypeId, setSelectedTypeId] = useState<string | null>(null);

  // Inline Classification Creation Modal
  const [showCreateClassificationModal, setShowCreateClassificationModal] = useState<boolean>(false);
  const [createClassTargetLevel, setCreateClassTargetLevel] = useState<1 | 2 | 3>(1);
  ...
  // --------------------------------------------------------------------------
  // CLASSIFICATION OPTIONS & INHERITED FIELDS COMPUTATION
  // --------------------------------------------------------------------------
  // Level 1 Categories (no parentId)
  ...
  // Level 2 Subcategories (parentId === selectedCatId)
  const subCategoryOptions: SelectOption[] = useMemo(() => {
    if (!selectedCatId) return [];
    return classificationsList
      .filter((c) => c.parentId === selectedCatId && c.isActive !== false)
      .map((c) => ({
        id: c.id,
        name: c.name,
        description: c.description,
      }));
  }, [classificationsList, selectedCatId]);

  // Level 3 Types (parentId === selectedSubCatId)
  const typeOptions: SelectOption[] = useMemo(() => {
    if (!selectedSubCatId) return [];
    return classificationsList
      .filter((c) => c.parentId === selectedSubCatId && c.isActive !== false)
      .map((c) => ({
        id: c.id,
        name: c.name,
        description: c.description,
      }));
  }, [classificationsList, selectedSubCatId]);

  // Fields defined specifically for the selected Type (Level 3)
  const inheritedFieldsList = useMemo(() => {
    const list: { levelLabel: string; field: AssetRequirementField }[] = [];

    // Fields belong exclusively to Type (Level 3)
    const typ = classificationsList.find((c) => c.id === selectedTypeId);
    if (typ?.fields) {
      typ.fields.forEach((f) =>
        list.push({ levelLabel: `فیلد اطلاعاتی نوع (${typ.name})`, field: f })
      );
    }
  ...
  // --------------------------------------------------------------------------
  // INLINE CLASSIFICATION CREATION HANDLER
  // --------------------------------------------------------------------------
  const handleOpenInlineCreateClassification = (level: 1 | 2 | 3) => {
    setCreateClassTargetLevel(level);
    setNewClassName('');
    setNewClassDescription('');
    setShowCreateClassificationModal(true);
  };

  const handleConfirmInlineCreateClassification = () => {
    if (!newClassName.trim()) return;

    let parentId: string | undefined = undefined;
    if (createClassTargetLevel === 2) {
      parentId = selectedCatId || undefined;
    } else if (createClassTargetLevel === 3) {
      parentId = selectedSubCatId || undefined;
    }

    const newSlug = `cls-${Date.now()}`;
    const newClassObj: Omit<AssetClassification, 'id' | 'createdAt' | 'updatedAt' | 'itemsCount'> = {
    ...
    onAddClassification(newClassObj);

    // Auto-select newly created item
    setTimeout(() => {
      if (createClassTargetLevel === 1) {
        setSelectedCatId(newClassObj.slug ? newClassObj.slug : `cls-${Date.now()}`);
        setSelectedSubCatId(null);
        setSelectedTypeId(null);
      } else if (createClassTargetLevel === 2) {
        setSelectedSubCatId(newClassObj.slug ? newClassObj.slug : `cls-${Date.now()}`);
        setSelectedTypeId(null);
      } else if (createClassTargetLevel === 3) {
        setSelectedTypeId(newClassObj.slug ? newClassObj.slug : `cls-${Date.now()}`);
      }
    }, 100);

    setShowCreateClassificationModal(false);
  };
  ...
  const handleStartManualRegistration = () => {
    ...
    // Reset 3-step hierarchy selection
  ...
  const handleSaveRegistration = (asDraft: boolean) => {
    ...
    const catObj = classificationsList.find((c) => c.id === selectedCatId);
    const subObj = classificationsList.find((c) => c.id === selectedSubCatId);
    const typeObj = classificationsList.find((c) => c.id === selectedTypeId);

    const categoryName = typeObj?.name || subObj?.name || catObj?.name || 'تجهیزات عمومی';
    const groupKey = `${formFaName || 'تجهیز جدید'} — ${formBrand || 'برند'} ${formModel || ''}`.trim();

    const newItem: EquipmentItem = {
      id: editingDraftItem ? editingDraftItem.id : `eq-${Date.now()}`,
      code: formCode.trim() || `EQ-DRAFT-${Math.floor(10 + Math.random() * 90)}`,
      faName: formFaName.trim() || 'تجهیز جدید',
      enName: formEnName.trim() || 'New Equipment Item',
    ...
    if (!isReallyDraft) {
      setToastMessage({
        ...
        subtitle: `کد دائم اموال: ${newItem.code} | محل استقرار: ${newItem.location} | برچسب QR فلزی صادر گردید.`,
        type: 'success',
      });
      if (activeGuidance?.type === 'draft_tagging') {
        setActiveGuidance(null);
  ...
  return (
    <div className="space-y-6 pb-16 font-sans dir-rtl text-right text-slate-800">
        ...
        <div className="rounded-3xl p-5 border shadow-xs transition-all animate-in fade-in slide-in-from-top-2 bg-gradient-to-r from-blue-50/95 via-sky-50/80 to-indigo-50/90 border-blue-200">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 shrink-0 self-end md:self-center">
              ...
              {activeGuidance.type === 'low_stock' && (
                <button
                  onClick={() => setShowQuickRestockModal(true)}
                  className="px-4 py-2.5 rounded-2xl bg-amber-600 hover:bg-amber-700 text-white text-xs font-black shadow-xs hover:shadow-md transition-all flex items-center gap-2 cursor-pointer"
                >
                  <Plus className="w-4 h-4" />
                  <span>ثبت رسید ورود و شارژ موجودی ❮</span>
                </button>
              )}
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
        ...
        {activeTab === 'inventory' && (
          <div className="p-5 space-y-5 animate-in fade-in">
            {/* 1. Quick Smart Automatic Filter Presets Bar */}
            <div className="p-3.5 bg-gradient-to-r from-blue-50/80 via-indigo-50/50 to-slate-50 rounded-2xl border border-blue-200/70 flex flex-col md:flex-row md:items-center justify-between gap-3 shadow-2xs">
              <div className="flex items-center gap-2 flex-wrap">
                <div className="flex items-center gap-1.5 text-xs font-black text-slate-800 shrink-0 ml-1">
                  <SlidersHorizontal className="w-4 h-4 text-blue-600" />
                  <span>فیلترهای خودکار هوشمند:</span>
                </div>
                ...
                >
                  ...
                  <span>بخش‌های ویژه و اورژانس</span>
                  <span
                    className={`text-[10px] px-1.5 py-0.2 rounded-md ${
              ...
              {activeFiltersCount > 0 && (
                <button
                  ...
                  className="text-[11px] font-bold text-rose-600 hover:text-rose-800 flex items-center gap-1 shrink-0 cursor-pointer bg-white px-2.5 py-1.5 rounded-xl border border-rose-200 hover:bg-rose-50 transition-colors shadow-2xs"
                >
                ...
                <div className="p-4 bg-slate-50/90 rounded-2xl border border-slate-200/90 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3.5 text-xs animate-in fade-in shadow-2xs">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="device">تجهیز و دستگاه سرمایه‌ای</option>
            ...
            <div className="p-5 space-y-3">
              ...
              >
                ...
                <div>
                  ...
                  <span className="text-[11px] text-slate-500 font-medium mt-0.5 block">
                    انتخاب ترتیبی: دسته بندی ← زیردسته ← نوع ← اطلاعات قلم
                  </span>
                </div>
              </div>
            </div>
      ...
      {/* ========================================================================= */}
      {/* 5. MODAL: MANUAL INVENTORY REGISTRATION (SEQUENTIAL 3-STEP DROPDOWNS)     */}
      {/* ========================================================================= */}
      {showManualModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-2xl w-full max-h-[92vh] overflow-hidden flex flex-col dir-rtl text-right animate-in fade-in">
            {/* Modal Header */}
            <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <div className="flex items-center gap-2">
                <FolderPlus className="w-5 h-5 text-[#2b64f6]" />
                <h3 className="text-sm font-bold text-slate-900">
                  ثبت تجهیز جدید (دسته‌بندی درختی)
                </h3>
              </div>
            ...
            {/* Form Body with Sequential Disclosure: Category -> Subcategory -> Type -> Fields */}
            <div className="p-5 overflow-y-auto space-y-4 text-xs">
              ...
              <div className="p-3.5 rounded-xl bg-blue-50/50 border border-blue-100 space-y-2">
                <SearchableSelect
                  ...
                  selectedValueId={selectedCatId}
                  onSelect={(id) => {
                    setSelectedCatId(id);
                    setSelectedSubCatId(null);
                    setSelectedTypeId(null);
                  }}
                  onCreateNew={() => handleOpenInlineCreateClassification(1)}
                  createNewText="+ ایجاد دسته بندی جدید"
                  required
                />
              </div>

              {/* STEP 2: زیردسته (Only shown if Category is selected) */}
              {selectedCatId && (
                <div className="p-3.5 rounded-xl bg-teal-50/40 border border-teal-100 space-y-2 animate-in fade-in">
                  <SearchableSelect
                    label="زیردسته"
                    placeholder="انتخاب زیردسته..."
                    options={subCategoryOptions}
                    selectedValueId={selectedSubCatId}
                    onSelect={(id) => {
                      setSelectedSubCatId(id);
                      setSelectedTypeId(null);
                    }}
                    onCreateNew={() => handleOpenInlineCreateClassification(2)}
                    createNewText="+ ایجاد زیردسته جدید"
                    required
                  />
                </div>
              )}

              {/* STEP 3: نوع (Only shown if Subcategory is selected) */}
              {selectedCatId && selectedSubCatId && (
                <div className="p-3.5 rounded-xl bg-indigo-50/40 border border-indigo-100 space-y-2 animate-in fade-in">
                  <SearchableSelect
                    label="نوع"
                    placeholder="انتخاب نوع قلم..."
                    options={typeOptions}
                    selectedValueId={selectedTypeId}
                    onSelect={(id) => setSelectedTypeId(id)}
                    onCreateNew={() => handleOpenInlineCreateClassification(3)}
                    createNewText="+ ایجاد نوع جدید"
                    required
                  />
                </div>
              )}

              {/* STEP 4: اطلاعات قلم (Only shown after Type is selected) */}
              {selectedCatId && selectedSubCatId && selectedTypeId && (
                <div className="space-y-4 pt-2 animate-in fade-in">
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                    <h4 className="text-xs font-extrabold text-slate-800 mb-3 flex items-center gap-1.5">
                      ...
                      <span>اطلاعات موردنیاز قلم (شامل فیلدهای موروثی و اختصاصی)</span>
                    </h4>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full p-2 rounded-lg bg-white border border-slate-200 text-slate-800"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full p-2 rounded-lg bg-white border border-slate-200 text-slate-800 font-mono"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full p-2 rounded-lg bg-white border border-slate-200 text-slate-800 font-mono font-bold"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <select
                          ...
                          className="w-full p-2 rounded-lg bg-white border border-slate-200 text-slate-800"
                        >
                          <option value="device">دستگاه / تجهیز سرمایه‌ای (تجهیزات پزشکی/اداری)</option>
                      ...
                      <div>
                        ...
                        <select
                          ...
                          className="w-full p-2 rounded-lg bg-white border border-slate-200 text-slate-800 font-semibold"
                        >
                          {formItemKind === 'device' ? (
                            <>
                              <option value="active">فعال و آماده به کار (Active)</option>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full p-2 rounded-lg bg-white border border-slate-200 text-slate-800 font-bold"
                        />
                      </div>
                      ...
                      {/* Render Dynamic Inherited Fields */}
                      {inheritedFieldsList.map(({ levelLabel, field }) => (
                        <div key={field.id} className="space-y-1">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-[9px] text-slate-400 font-normal">{levelLabel}</span>
                          </div>
                          ...
                          {field.type === 'select' && field.options ? (
                            <select
                              ...
                              className="w-full p-2 rounded-lg bg-white border border-slate-200 text-slate-800 text-xs"
                            >
                ...
                selectedCatId && selectedSubCatId && selectedTypeId && (
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="px-5 py-2 rounded-xl bg-[#2b64f6] hover:bg-blue-700 text-white text-xs font-extrabold transition-all shadow-xs cursor-pointer"
                    >
                      ثبت نهایی
                    </button>
                  </div>
      ...
      {/* ========================================================================= */}
      {/* 6. MODAL: INLINE CLASSIFICATION CREATION (+ ایجاد جدید)                 */}
      {/* ========================================================================= */}
      {showCreateClassificationModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl max-w-md w-full p-5 space-y-4 dir-rtl text-right animate-in fade-in">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-xs font-extrabold text-slate-900">
                {createClassTargetLevel === 1
                  ? 'ایجاد دسته بندی جدید'
                  : createClassTargetLevel === 2
                  ? 'ایجاد زیردسته جدید'
                  : 'ایجاد نوع جدید'}
              </h3>
              <button
                onClick={() => setShowCreateClassificationModal(false)}
            ...
            <div className="space-y-3 text-xs">
              <div>
                ...
                <input
                  ...
                  placeholder="مثلا: تجهیزات لیزری و زیبایی"
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="توضیح مختصر در مورد این رده..."
                  className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-800 text-xs focus:outline-none focus:border-[#2b64f6]"
                />
              </div>
            </div>
            ...
            <div className="pt-2 flex items-center justify-end gap-2 border-t border-slate-100">
              <button
                onClick={() => setShowCreateClassificationModal(false)}
                className="px-3.5 py-2 rounded-xl bg-slate-100 text-slate-600 font-semibold hover:bg-slate-200"
              >
                انصراف
              </button>
              <button
                onClick={handleConfirmInlineCreateClassification}
                disabled={!newClassName.trim()}
                className="px-4 py-2 rounded-xl bg-[#2b64f6] hover:bg-blue-700 disabled:bg-slate-300 text-white font-extrabold shadow-2xs"
              >
                ایجاد و انتخاب خودکار
              </button>
            </div>
          </div>
        </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full p-2.5 rounded-xl bg-white border border-slate-200 text-slate-800 font-bold focus:border-[#2b64f6] focus:outline-none"
                    >
                      <option value="device">دستگاه و تجهیز سرمایه‌ای</option>
