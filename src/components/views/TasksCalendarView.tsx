...
import { AppUser, TaskEvent, PageId } from '../../types';
import {
  getWorkgroupMembers,
  getTaskTypesForRole,
...
export const TasksCalendarView: React.FC<TasksCalendarViewProps> = ({
  ...
  const roleChecklistDescriptions: Record<string, string> = {
    hospital_admin: 'چک‌لیست نظارتی ارشد: تاییدات نهایی درخواست‌های خرید، ارزیابی بودجه و بررسی چک‌لیست‌های اعتباربخشی بیمارستان',
    support_tech: 'چک‌لیست خدمات و پشتیبانی فنی: تیکت‌های تعمیرات حضوری، سرویس‌های پیشگیرانه و رفع خرابی‌های ثبت‌شده',
    procurement_officer: 'چک‌لیست بازرگانی و خرید: استعلام قیمت از تامین‌کنندگان، پیگیری سفارشات و تاییدات خرید و سبد هوشمند',
    biomedical_engineer: 'چک‌لیست مهندسی پزشکی: آزمون‌های دوره‌ای کالیبراسیون، کنترل کیفی، تست‌های ایمنی الکتریکی و شناسنامه فنی',
    dept_head: 'چک‌لیست سرپرستی بخش: تحویل شیفت تجهیزات، درخواست‌های نوسازی و پیگیری خرابی‌های اورژانسی بخش',
    asset_manager: 'چک‌لیست اموال و دارایی‌ها: پلاک‌کوبی بارکد اموال، انبارگردانی فیزیکی و حسابرسی استهلاک دارایی‌ها',
    nurse_operator: 'چک‌لیست پرستاری و اپراتوری: چک‌لیست روزانه تحویل شیفت، تست دفیبریلاتور ترالی کد و گزارش سریع اشکالات',
    warehouse_keeper: 'چک‌لیست انبار مرکزی: کنترل موجودی ورود/خروج قطعات یدکی، تطبیق بارنامه و سفارش قطعات کمبود',
    finance_manager: 'چک‌لیست امور مالی: بررسی و پرداخت صورت‌حساب‌های تامین‌کنندگان، تایید مالی پیش‌فاکتورها و تحلیل بودجه',
  };

  const currentRoleDesc =
    (currentUser?.role && roleChecklistDescriptions[currentUser.role]) ||
    'چک‌لیست روزانه، پیگیری کالیبراسیون‌ها، سرویس‌های دوره‌ای و ماموریت‌های عملیاتی';
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">{currentRoleDesc}</p>
        </div>
      ...
      <div className="bg-gradient-to-r from-sky-50 via-indigo-50/50 to-blue-50 border border-sky-200/80 p-5 rounded-3xl space-y-4 shadow-2xs">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-white text-sky-600 border border-sky-100 flex items-center justify-center shrink-0 shadow-xs">
              <UserCheck className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-black text-slate-800">
                  چک‌لیست فعال: {currentUser?.name || 'کاربر سیستم'}
                </span>
                <span className="text-[11px] px-2.5 py-0.5 rounded-full bg-sky-100 text-sky-800 font-bold border border-sky-200">
                  {currentUser?.roleFa || 'کاربر'}
                </span>
              </div>
              <p className="text-[11px] text-slate-600 mt-0.5">
                تسک‌ها بر اساس صلاحیت شغلی و دامنه فعالیت این نقش فیلتر شده‌اند.
              </p>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-lg w-full border border-slate-200 shadow-2xl p-6 space-y-5 animate-in fade-in zoom-in-95 duration-150 text-right">
            ...
            <div className="space-y-2 pt-2 border-t border-slate-100">
              <button
                onClick={() => {
                  onToggleTaskStatus(selectedTask.id);
