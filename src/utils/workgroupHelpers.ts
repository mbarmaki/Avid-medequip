...
export function getTaskTypesForRole(role?: UserRole): WorkgroupTaskType[] {
    ...
    return [
      {
        ...
        description: 'الصاق پلاک اموال، بارکدگذاری فیزیکی و تطبیق اطلاعات شاسی تجهیزات جدید و پیش‌نویس',
      ...
      {
        id: 'stock_check',
        label: 'کنترل موجودی، رسید ورود کالا و کسری انبار',
        description: 'ثبت ورود قطعات/کالاهای مصرفی، پایش نقطه سفارش و ترخیص از قرنطینه',
        badgeColor: 'bg-amber-100 text-amber-800 border-amber-200',
      },
      {
        id: 'asset_transfer',
        label: 'صورت‌جلسه تحویل، تحول و جابجایی اموال بین بخش‌ها',
        description: 'انتقال تجهیزات بیمارستانی بین بخش‌ها و امضای الکترونیک صورت‌جلسه تحویل',
  ...
  return [
    ...
    },
    {
      ...
      description: 'نظارت بر کاردکس انبار مرکزی و رسیدهای کالا',
      badgeColor: 'bg-amber-100 text-amber-800 border-amber-200',
    },
    {
      id: 'asset_transfer',
...
export function getNotificationsFromTasksAndEvents(
  ...
  // 1. Add draft equipment items that require tagging for Asset Manager
  if (userRole === 'asset_manager' || userRole === 'warehouse_keeper' || userRole === 'hospital_admin') {
    ...
    if (draftItems.length > 0) {
      ...
      const draftTask: TaskEvent = {
        ...
        title: `پلاک‌کوبی و صدور کد اموال برای ${firstDraft.faName}`,
        type: 'tagging',
        priority: 'high',
        ...
        status: 'open',
        autoGenerated: true,
        notes: 'تکمیل فیلدهای الزامی کد اموال، شماره سریال و استقرار فیزیکی',
      };

      notifs.push({
        id: `notif-draft-tagging-${firstDraft.id}`,
        taskId: `draft-${firstDraft.id}`,
        sourceType: 'draft',
        targetEquipmentId: firstDraft.id,
        title: `تکمیل شناسنامه و پلاک‌کوبی ${draftItems.length} قلم پیش‌نویس`,
        desc: `تجهیز «${firstDraft.faName}» (${firstDraft.category}) فاقد پلاک کد اموال قطعی است و نیاز به تکمیل دارد.`,
        time: 'مهلت اقدام: امروز',
        type: 'warning',
        priority: 'high',
        isDraftTagging: true,
  ...
  // 2. Add real tasks as notifications
